...
diag_with_admits %>% group_by(SUBJECT_ID, ICD9_CODE) %>% count %>% mutate(x="C") %>% 
  ggplot(aes(x=x, y=n)) + 
  geom_violin() + 
  ylim(c(0,10))
...
Data is mostly complete; only 47 patients have no ICD code 
...
save(ICD9_depth, file = "Rdata/ICD9_depth.Rdata")
```
