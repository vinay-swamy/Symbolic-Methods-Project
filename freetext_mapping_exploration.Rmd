...
```{r}
ft_notes_mapped_to_ICD9_clean %>% left_join(diag_with_admits %>% select(SUBJECT_ID, HADM_ID, ICD9_CODE) %>% mutate(code_type = 'Matches Billed Code')) %>%
  ...
  group_by(code_type) %>% 
  count %>% 
  ggplot(aes(x="",y=n, fill = code_type)) + 
  geom_bar(stat = "identity") + 
  coord_polar("y", start=0) + 
  ggtitle(paste0("n = ", nrow(ft_notes_mapped_to_ICD9_clean), " ICD9 Codes" )) +
...
pv = tibble(x= 1, y=18000, label= wilcox.test(diag_ICD9_with_ft_ICD9_with_cost %>% filter(CODE_ORIGIN == "freetext_note") %>% pull(avg_estim_dollar_sub), 
            diag_ICD9_with_ft_ICD9_with_cost %>% filter(CODE_ORIGIN != "freetext_note") %>% pull(avg_estim_dollar_sub)
            )[['p.value']] %>% signif(3) %>% {paste0("Wilcox p value = ",.)})
...
ggplot(diag_ICD9_with_ft_ICD9_with_cost %>% 
  ...
  geom_label(data = pv ,aes(x=x, y=y, label= label))+
  ylim(c(0, 20000)) + 
  xlab("")+
